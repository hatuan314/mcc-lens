...
- **Primary:**
  ...
  - LLM đóng vai trò Giám khảo (LLM-as-judge) để đánh giá độ phù hợp và trả về điểm số (score) từ 0.0 đến 1.0.
  - Xuất **2 file Excel**:
...
- **Secondary:**
  - CLI subcommand `python3 main.py map-vsic-mcc` với `--vsic-input`, `--mcc-input`, `--output`, `--output-detail`, `--resume`, `--top-k` (default 60).
...
- [ ] Score là float [0, 1] (làm tròn đến 2 chữ số thập phân); Nhận xét là string tóm tắt mức phù hợp.
...
- [ ] Giá trị `Nhận xét` và `Score` cho mỗi rank: **do LLM (Qwen2.5:14b) sinh ra** — 1 câu ngắn giải thích và điểm số độ phù hợp (0.0 đến 1.0). Được trả về cùng prompt re-rank, không tốn thêm lần gọi LLM riêng. Nếu LLM lỗi, fallback về điểm Cosine Similarity. → **Resolved**
