...
- **Colab pip conflicts**: Trên Colab, dùng `colab/requirements-mapping.txt` thay vì `requirements.txt` để tránh cài `surya-ocr`/dev tools gây xung đột `httpx`/`PIL` và bắt restart runtime.
